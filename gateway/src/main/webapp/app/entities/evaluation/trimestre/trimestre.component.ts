...
export class TrimestreComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.trimestreService
      ...
      .subscribe(
        (res: HttpResponse<ITrimestre[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInTrimestres(): void {
    this.eventSubscriber = this.eventManager.subscribe('trimestreListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITrimestre[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
