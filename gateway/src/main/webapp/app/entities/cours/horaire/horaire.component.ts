...
export class HoraireComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.horaireService
      ...
      .subscribe(
        (res: HttpResponse<IHoraire[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInHoraires(): void {
    this.eventSubscriber = this.eventManager.subscribe('horaireListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IHoraire[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
